...
Отдельное мини-приложение с карточками «Да / Нет» на четырёх языках: `/quiz/uk` (главный), `/quiz/en`, `/quiz/fr`, `/quiz/ru`. Не использует ничего из основного сайта: свой layout, шрифты, стили и тексты.

- Код: `src/quiz/` (компоненты, контент, настройки) и `src/app/quiz/` (страницы и стили).
- Тексты всех языков: `src/quiz/content/{uk,en,fr,ru}.ts`. У вопроса есть заголовок и необязательный длинный текст `body`, который прокручивается в своей колонке.
- Выключить: переменная сборки `QUIZ_ENABLED=false` — все страницы опросника отдают 404.
- Удалить совсем: папки `src/quiz/` и `src/app/quiz/` и `public/quiz/` (страница-перенаправление с `/quiz` на `/quiz/uk`).
